...
public class ProjectService {
	...
	private CategoryResponse  getCategoryResponse(category_project  _category_projectBdd) {
		
		
		CategoryResponse  _categoryResponse = new CategoryResponse();
		
		_categoryResponse.setToken(_category_projectBdd.getToken());
		
		_categoryResponse.setNom(_category_projectBdd.getNom());
		
		_categoryResponse.setNbr_projects(_category_projectBdd.getNbr_projects());
	...
	private StatutResponse  getStatutResponse(statutProject  _statut_projectBdd) {
		
		
		StatutResponse  _statutResponse = new StatutResponse();
		
		_statutResponse .setToken(_statut_projectBdd.getToken());
		
		_statutResponse .setNom(_statut_projectBdd.getNom());
		
		return _statutResponse;
		
		
		
	}
	
	private CommentProjectResponse getCommentProjectResponse(commentProject  _comment_projectBdd,UserResponse _user) {
		
		
		CommentProjectResponse  _comment_response = new CommentProjectResponse();
		
		_comment_response.setBodyComment(_comment_projectBdd.getBodyComment());
		
		_comment_response.setDateCreated(_comment_projectBdd.getDateCreated());
		
		_comment_response.setToken(_comment_projectBdd.getToken());
		
		_comment_response.setTimestamp(_comment_projectBdd.getTimestamp());
		
		_comment_response.setUser(_user);
		
		return _comment_response;
	}
	
	private NewsProjectResponse getNewsProjectResponse(newsProjectModel  _news_projectBdd) {
		
		
		NewsProjectResponse  _news_response = new NewsProjectResponse();
		
		_news_response.setDescription(_news_projectBdd.getDescription());
		
		_news_response.setPhotos(_news_projectBdd.getPhotos());
		
		_news_response.setTitre(_news_projectBdd.getTitre());
		
		_news_response.setToken(_news_projectBdd.getToken());
		
		_news_response.setTimestamp(_news_projectBdd.getTimestamp());
		
		_news_response.setDate_created(_news_projectBdd.getDate_created());
		
		return _news_response;
	}
	
	
	private LinkImageProjectResponse getImageProjectResponse(linkImageProject  _image_projectBdd) {
		
		
		LinkImageProjectResponse  _image_response = new LinkImageProjectResponse();
		
		_image_response.setToken(_image_projectBdd.getToken());
		
		_image_response.setLink(_image_projectBdd.getLink());
		
		return _image_response;
	}
	
	
	private AdressReseauxSociauxProjectResponse   getAdressReseauxSociauxProjectResponse(adressReseauxSociauxProject  _adressReseauxSociauxBdd) {
		
		AdressReseauxSociauxProjectResponse  _adressReseauxSociauxProjectResponse = new AdressReseauxSociauxProjectResponse();
		
		_adressReseauxSociauxProjectResponse.setKeyMedia(_adressReseauxSociauxBdd.getKeyMedia());
		
		_adressReseauxSociauxProjectResponse.setLinkProject(_adressReseauxSociauxBdd.getLinkProject());
		
		_adressReseauxSociauxProjectResponse.setToken(_adressReseauxSociauxBdd.getToken());
		
		_adressReseauxSociauxProjectResponse.setValueMedia(_adressReseauxSociauxBdd.getValueMedia());
		
		return _adressReseauxSociauxProjectResponse;		
	...
	private ProjectResponseForVisitor getProjectResponse(project   _project_Bdd,UserResponse _user) {
		
		 ProjectResponseForVisitor _project_response = new  ProjectResponseForVisitor();

         _project_response.setNom(_project_Bdd.getNom()); 

         _project_response.setToken(_project_Bdd.getToken());

         _project_response.setAfficheProject(_project_Bdd.getAfficheProject());
         
         _project_response.setAfficheProject(_project_Bdd.getAfficheProject());
         
         _project_response.setMontant_minimun(_project_Bdd.getMontant_minimun());
         
         _project_response.setTotal_comments(_project_Bdd.getTotal_comments());
         
         _project_response.setTotal_dislike(_project_Bdd.getTotal_dislike());
         
         _project_response.setTotal_favoris(_project_Bdd.getTotal_favoris());
         
         _project_response.setTotal_fonds(_project_Bdd.getTotal_fonds());
         
         _project_response.setTotal_hearts(_project_Bdd.getTotal_hearts());
         
         _project_response.setTotal_investisseurs(_project_Bdd.getTotal_investisseurs());
         
         _project_response.setTotal_like(_project_Bdd.getTotal_like());
         
         _project_response.setTotal_vues(_project_Bdd.getTotal_vues());
         
         _project_response.setCreated_at(_project_Bdd.getCreated_at());
         
         _project_response.setCategoryProject(this.getCategoryResponse(_project_Bdd.getCategoryProject()));
         
         _project_response.set_porte_project(_project_Bdd.get_porte_project());
         
         _project_response.setDate_limite_collecte(_project_Bdd.getDate_limite_collecte());
         
         _project_response.setDescription(_project_Bdd.getDescription());
         
         _project_response.set_statut_project(this.getStatutResponse(_project_Bdd.get_statut_project()));

         _project_response.set_user(_user );
	...
	public project updateProject(@CurrentUser UserPrincipal currentUser, String token_project,
			@Valid @RequestBody ProjetRequest updateProject) {
		...
		if (projectBdd.isPresent()) {
			...
			if (isEqual) {
				...
				projectBddUpdate.setCategoryProject(updateProject.getCategoryProject());

				projectBddUpdate.set_porte_project(updateProject.get_porte_project());
	...
	public AdressReseauxSociauxProjectResponse createAdressReseauSocialProject(@CurrentUser UserPrincipal currentUser,String token_project, @Valid @RequestBody AdressReseauxSociauxProjectRequest _linkSocialReq) {
		...
		if (_project.isPresent()) {
			...
			if (isEqual) {
				...
				AdressReseauxSociauxProjectResponse _linkSocialResp = this.getAdressReseauxSociauxProjectResponse(_linkSocialBddupdate);			
			...
			List<LinkImageProjectResponse> _links = new ArrayList<LinkImageProjectResponse>();
				...
				if (_linksBdd.size() > 0) {
					...
					for (int index = 0; index < _linksBdd.size(); index++) {
						...
						_links.add(this.getImageProjectResponse(_linksBdd.get(index)));
	...
	public LinkImageProjectResponse createLinkImageProject(@CurrentUser UserPrincipal currentUser, String token_project,@Valid @RequestBody LinkImageRequest _linkImageReq) {
		...
		if (_project.isPresent()) {
			...
			if (isEqual) {
				...
				/**********************************************************/
				LinkImageProjectResponse _linkImageResp = this.getImageProjectResponse(_linkImageProjectBddupdate);					
		...
		if (_project.isPresent()) {
			...
			if ( _commentsBdd.size() > 0) {
				...
				for (int index = 0; index <  _commentsBdd.size(); index++) {
					...
					User _user = _commentsBdd.get(index).get_user();
					
					_comments.add(this.getCommentProjectResponse(_commentsBdd.get(index),this.getUserResponse(_user)));				
	...
	public CommentProjectResponse createCommentProject(@CurrentUser UserPrincipal currentUser, String token_project,
		...
		if (_project.isPresent()) {
			...
			User _user = _commentProjectBddupdate.get_user();
			
			CommentProjectResponse    _commentProjectResp =  this.getCommentProjectResponse(_commentProjectBddupdate,this.getUserResponse(_user));	
	...
	public List<ProjectResponseForVisitor> findAllProjectsByLikeTag(String tag) {		
		...
		for (int i = 0; i < _projects_Bdd.size(); i++) {			
			 ...
			 UserResponse    _new_user =  this.getUserResponse(_projects_Bdd.get(i).get_user()); 		
			
			ProjectResponseForVisitor _project_response = this.getProjectResponse(_projects_Bdd.get(i),_new_user);
  ...
  public  ProjectResponseForVisitor  getRandomProject() {		
		  ...
		  UserResponse    _new_user = this.getUserResponse(_project_Bdd.get_user());		  
          
          ProjectResponseForVisitor _project_response = this.getProjectResponse(_project_Bdd,_new_user);
	      ...
	      List<commentProject> _comments = _commentProjectRepository.findBy_project(_project_Bdd);
			...
			if (_comments.size() > 0) {
				...
				for (int index = 0; index < _comments.size(); index++) {					
					  ...
					  UserResponse    _user_comment = this.getUserResponse(_comments.get(index).get_user());				
					
					
					  CommentProjectResponse  _comment_response = this.getCommentProjectResponse(_comments.get(index),_user_comment);			
	      ...
	      /************************list news-project ********************************/
			...
			if (_listNewsProjectsBdd.size() > 0) {
				...
				for (int index = 0; index < _listNewsProjectsBdd.size(); index++) {
					...
					NewsProjectResponse  _news_response = this.getNewsProjectResponse(_listNewsProjectsBdd.get(index));				
			...
			if (_links.size() > 0) {
				...
				for (int index = 0; index < _links.size(); index++) {
					...
					LinkImageProjectResponse  _link_image_response = this.getImageProjectResponse(_links.get(index));				
