...
public interface  messageRepository extends JpaRepository<messageModel, Long> {
	 ...
	 @Query(value = "SELECT  *  FROM  messagerie_interne  WHERE  _user_dest=?1", nativeQuery = true)
	 List<messageModel> findAllMessagesRecusByTokenUser(String tokenUser);
	 
	 
	 @Query(value = "SELECT  *  FROM  messagerie_interne  WHERE  _user_exp=?1", nativeQuery = true)
	 ...
	 @Query(value = "SELECT  COUNT(*)  FROM  messagerie_interne  WHERE  _user_dest=?1   AND date_consultation IS  NULL", nativeQuery = true)
	 int  countMessageNonLusByTokenUser(String tokeuser);
	 
	 @Query(value = "SELECT  * FROM  messagerie_interne  WHERE  _user_dest=?1   AND date_consultation IS  NULL  ORDER BY timestamp DESC   LIMIT 4", nativeQuery = true)
