...
public interface  heartsProjectRepository extends JpaRepository<heartProjectModel, Long> {
	 ...
	 Optional<heartProjectModel>  findCheckHeartProject(String tokenUser,String tokenProject);
	 ...
	 @Query(value = "SELECT  COUNT(*)  FROM  hearts_project_user   WHERE   token_project=?1  AND token_user=?2", nativeQuery = true)
	 int   countHeartByProjectByUser(String tokenProject,String tokenUser);
     ...
     @Query(value = "SELECT  *  FROM  hearts_project_user   WHERE  token_project=?1 ", nativeQuery = true)
	 List<heartProjectModel>  findAllHeartsProject(String tokenProject);
